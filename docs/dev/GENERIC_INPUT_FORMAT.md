...
For orientation searches generated by the Python wizard:

- `orient_1d`: candidate rows rotate the current orientation around one axis.
- `orient_2d`: candidate rows are a 2D grid over two angle ranges.

Candidate row formats by type:

- **Point / Pin**: `[x, y, z, nx, ny, nz]`
- **Line**: `[mx, my, mz, lx, ly, lz, nx, ny, nz, length]`
- **Plane**: `[px, py, pz, nx, ny, nz, type, ...prop]`
