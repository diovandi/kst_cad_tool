...
> **Note (2026):** This file is generated by `scripts/deep_comparison.py` and may include **historical diagnostic text** at the end (e.g. “Root cause notes”). The **current codebase matches Octave/MATLAB for all 21 benchmark cases** (see `docs/dev/PROJECT_STATUS_SUMMARY.md` and `python scripts/compare_octave_python.py all`). Treat the closing “Root cause notes” section as a **snapshot from an earlier parity investigation**, not the present state.
...
## Root cause notes (Python vs MATLAB/Octave) — historical

> **Superseded:** The issues below were investigated during parity work. **As of 2025–2026, Python matches Octave/MATLAB for all 21 cases** within project tolerances. Regenerate this file after changing results; do not rely on this subsection alone for current status.
