...
- **Other cases (6–21):** Thesis Ch 10/11 do not give explicit baseline tables for every input file (e.g. case3b/3c, endcap, printer variants). For those, validation is done by **Python vs Octave** comparison (`compare_octave_python.py all`). **Python matches Octave for all 21 cases** (atol=1e-3, rtol=5%); see [PARKED.md](../validation/PARKED.md) and [PROJECT_STATUS_SUMMARY.md](../dev/PROJECT_STATUS_SUMMARY.md).
