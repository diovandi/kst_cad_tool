...
def test_analyze_constraints_duplicate_detection(mock_pipeline_dependencies):
    ...
    with patch("kst_rating_tool.pipeline.Pool") as mock_pool_cls:
        ...
        assert res_par.no_mot_half == 3
        assert list(res_par.combo_dup_idx) == [0, 1, 0, 0]
...
def test_analyze_constraints_detailed_indices(mock_pipeline_dependencies):
    ...
    with patch("kst_rating_tool.pipeline.Pool") as mock_pool_cls:
        ...
        # Deliberately return out-of-order rows; function should sort by combo index.
        chunk_data = [
            [
                (2, r_c, dummy_R),
                (0, r_a, dummy_R),
        ...
        assert res_par.no_mot_half == 2
        # combo_proc should include accepted combos in parallel mode as [combo_idx(1-based), c1..c5]
        assert res_par.combo_proc.shape == (2, 6)
        assert list(res_par.combo_proc[:, 0]) == [1, 3]
